...
for THRESHOLD in THRESHOLDs:
    ...
    df_sub = pd.read_csv('fish_crop_preds.csv')
    ...
    with tqdm(total = df_sub.shape[0]) as pbar:
        for index, row in df_sub.iterrows():
            ...
            if((block_start_idx > -1) & (is_block==0) & (df_sub.loc[current_idx:current_idx+2,'xmin'].sum()==0) & (current_vid == previous_vid)):
    ...
    df_sub.to_csv('ssd_fish_thres_max_all_class_crop_{}_2_frame.csv'.format(THRESHOLD))
